...
int IsFull(void) {
  return ((_rx_buffer->head + 1) % UART_BUFFER_SIZE) == _rx_buffer->tail;
}

void Uart_isr ()
{
    /* if DR is not empty and the Rx Int is enabled */
    if ((((USART1->SR >> 5)&1) != 0) &&(((USART1->CR1 >> 5)&1) != 0))
    {
    	 /******************
        ...
        unsigned char c = USART1->DR;     /* Read data register */
        store_char (c, _rx_buffer);  // store data in buffer
        return;
    }

    /*If interrupt is caused due to Transmit Data Register Empty */
    if ((((USART1->SR >> 7)&1) != 0) &&(((USART1->CR1 >> 7)&1) != 0))
    {
